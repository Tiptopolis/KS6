...
import prime.PRIME.SYS.ECS.aComponent;
...
public class TestApp1 extends uApp {
	...
	public void updateMesh(float deltaTime) {
		...
		this.rotate(T, (new Vector3(0, 0, 1)), rotVelocity);
	...
	public void rotate(Transform t, Vector3 axis, float degrees) {
		Vector3 d = downcast(t.getLocalRotation());

		if (isEqual(d, Vector3.Zero, 0.001f))
			d.set(downcast(Transform.SpatialBasis.getLocalRotation()));

		d.rotate(axis, degrees);
		t._localNormal.rotate(axis, degrees);
		t.setLocalRotation(upcast(d));
		for (Transform c : t.getChildren()) {
			rotate(c, axis, degrees);
			rotAround(c, axis, degrees);
		}
	}

	public void rotAround(Transform c, Vector3 axis, float degrees) {
