...
import prime.PRIME.SYS.Graph.aNode;
import prime.PRIME.SYS.Graph.aNodeBuilder;

public class zL1_1 extends uApp {
	...
	public void create() {
		...
		this.bogus = new _Environment("ENV") {
			@Override
			public Vector3 getUnit() {
				return new Vector3(32f, 32f, 32f);
			}
		};
	...
	public void update(float deltaTime) {
		...
		if (this.testRay != null) {
			Log(this.testRay.toString());
			Log(this.testRay.toLog());
		}
	}
	...
	public void render() {
		...
		if (this.testRay != null)
			this.testRay.render(this.Explorer.perspective.camera);
	...
	public void genMesh() {
		...
		Log(aNodeBuilder.createNode("N5", Mesh.class, _VTS.Global.getA("Mesh")));
	...
	public void updateMesh(float deltaTime) {
		...
		T2.move(new Vector3((int) ((iTime) + (Math.signum(iTime) * 0.5f)), 0, 0));
	...
	public boolean keyDown(int keycode) {
		// Log(this.getClass().getSimpleName() + "-> " + keycode);
		// if (Gdx.input.isKeyPressed(Keys.SHIFT_LEFT)) {
		if (keycode == Keys.EQUALS) {
			Log("Spawn Ray");
			// this.spawnCamGhost();
			this.spawnRay(this.Explorer.perspective.camera);
		}
		if (keycode == Keys.MINUS) {
			Log("Despawn Ray");
			this.despawnRay();
			// this.camGhosts.clear();
		}
	...
	public void spawnRay(Camera c) {
		if (this.testRay != null) {
			testRay.clear();
			testRay.cast(c.position, c.direction, new Vector3(far, far, far),
					this.Explorer.perspective.camera.up.cpy());
		} else
			this.testRay = new lRay(this.bogus, c.position, c.direction, this.Explorer.getCameraUp(), far);
	}

	public void despawnRay() {
		testRay.clear();
		testRay.dispose();
	}
